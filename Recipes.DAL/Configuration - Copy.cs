...
namespace Recipes.DAL.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<DataContext>
    {
        public Configuration()
        {
            ...
            //Database.SetInitializer<DataContext>(new SeedOnlyInitializer());
        }
        ...
        void SeedShoppingList(Recipes.DAL.Data.DataContext context)
        {
            ...
            context.ShoppingLists.AddOrUpdate(x => x.Text, shoppingList);
            context.SaveChanges();
            shoppingList = context.ShoppingLists.Include(x => x.Groups).First();

            if (context.ShoppingListGroups.Count() == 0)
            {
        ...
        void SeedPlanner(DataContext context)
        {
            ...
            context.Planners.AddOrUpdate(x => x.Text, planner);
            context.SaveChanges();
            planner = context.Planners.Include(x => x.Groups).First();

            if (planner.Groups.Count == 0)
            {
        ...
        void SeedTags(Recipes.DAL.Data.DataContext context)
        {
